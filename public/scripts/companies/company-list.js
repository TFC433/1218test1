...
// (V-Style-Upgrade: 採用精簡標籤感設計並整合機會數計算)
...
async function loadCompaniesListPage() {
    ...
    try {
        // 2. 併發獲取數據：方案 A - 抓取所有機會案件來計算公司機會數
        ...
        html += `
            <tr>
                ...
                <td class="col-actions">
                    <button class="btn-mini-delete" title="刪除公司" onclick="confirmDeleteCompany('${encodedName}')">
...
async function confirmDeleteCompany(encodedName) {
    ...
    showConfirmDialog(`確定要永久刪除公司「${name}」及其所有關聯資料嗎？`, async () => {
        ...
        try {
            ...
            } else { throw new Error(res.error || '刪除失敗'); }
